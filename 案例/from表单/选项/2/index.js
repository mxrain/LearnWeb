// 获取页面中需要操作的元素，包括下拉菜单标题图标、标题文本、下拉列表、主按钮以及浮动图标。
// 需要操作到的元素   
...
// 定义了一个 icons 对象，用来存储不同平台的图标路径（SVG path 数据），方便后面动态生成图标。
// 定义了一个 list_items 数组，用来存储下拉菜单中显示的平台名称。
...
// 定义了 iconTemplate 和 listItemTemplate 两个函数，用来生成 SVG 图标和下拉列表项的 HTML 代码。
...
// renderListItems 函数使用 listItemTemplate 函数动态生成下拉列表中的所有平台项，并在页面加载时执行。
// 生成下拉列表项
...
// setDropdownProps 函数用来设置 CSS 自定义属性的值，包括下拉菜单展开状态、高度、列表透明度等。

// 设置自定义属性的值
const setDropdownProps=(deg,ht,opacity)=>{
    root.style.setProperty("--rotate-arrow",deg!==0?deg+"deg":0); // 设置旋转角度
    root.style.setProperty("--dropdown-height",ht!==0?ht+"rem":0); // 设置高度
    root.style.setProperty("--list-opacity",opacity); // 设置列表透明度
}

// 下拉框按钮点击事件
main_button.addEventListener("click",()=>{
    const list_wrapper_sizes=3.5; // 列表外层容器高度
    const dropdown_open_height=4.6*list_items.length+list_wrapper_sizes; // 计算展开后的高度
    const curr_dropdown_height=root.style.getPropertyValue("--dropdown-height")||0; // 获取当前高度
    curr_dropdown_height==="0"?setDropdownProps(180,dropdown_open_height,1):setDropdownProps(0,0,0);  // 根据当前高度设置属性值
})

// 监听下拉列表项的鼠标移入事件，并使用 --translate-value 自定义属性来控制浮动图标的横向移动位置。

// 下拉列表项鼠标移入事件
dropdown_list.addEventListener("mouseover",(e)=>{ // 监听鼠标移入事件
    const translate_value=e.target.dataset.translateValue; // 获取当前鼠标移入的元素 dataset 属性值
    root.style.setProperty("--translate-value",translate_value); // 设置自定义属性值
})
...
dropdown_list.addEventListener("mousemove",(e)=>{
    ...
    const targetText=e.target.innerText.toLowerCase().trim();// 获取当前鼠标移入的元素 dataset 属性值
    const hover_item_text=icons[targetText]; // 获取当前鼠标移入的元素 dataset 属性值
    ...
    console.log(targetText,x-icon_size/2, y-icon_size/2);
